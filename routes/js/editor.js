var app = new Vue({
    ...
    methods: {
        updateTabs: function(files) {
            ...
            this.$nextTick(() => {
                document.querySelector(`[tab-key="${this.filekey}"]`).classList.add('mdc-tab--active');
                document.querySelector(`[tab-key="${this.filekey}"] .mdc-tab-indicator`).classList.add('mdc-tab-indicator--active');
                loadcomponents();
            })
        },
        getRecent: function() {
